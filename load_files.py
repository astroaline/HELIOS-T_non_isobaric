...
def load_opacity_data(molecule):
    ...
    wavenumber_min = wavelength_bins[-1]
    wavenumber_max = wavelength_bins[0]
...
def load_sigma(molecule1, molecule2, x_full): 
    ...
    if pad_start > 0:
        cia_data = np.array(cia_data)[:, pad_start:]
    else:
        cia_data = np.pad(cia_data, ((0, 0), (-pad_start, 0)), 'constant')

    if pad_end < 0:
        cia_data = np.array(cia_data)[:, :pad_end]
    else:
