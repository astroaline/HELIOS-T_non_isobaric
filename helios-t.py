...
#np.save('retrieved_results_' + planet_name + '_' + model_name, retrieved_results, log_evidence)

#save_retrieved_results = open('retrieved_results_'+planet_name+'_'+model_name, 'w')
#np.savetxt(save_retrieved_results, retrieved_results+log_evidence)

with open('/home/aline/Desktop/PhD/HELIOS-T-master_[non-isobaric]/planets/'+planet_name+'/retrieved_results_'+planet_name+'_non_isobaric_'+model_name+'.txt', 'w') as save_retrieved_results:
    for param in parameters:
...
y = model.Model(len_x, x_full, bin_indices, new_param_dict, integral_grid)
...
fig = cornerplot.corner(samples, x_full_plot, wavelength_centre, yfit, yfit_binned, ydata, wavelength_err, yerr, posterior_ranges, color_list, labels=retrieved_parameter_labels, truths=plot_percentiles, max_n_ticks=3, label_kwargs={"fontsize": 40})

fig.savefig('/home/aline/Desktop/PhD/HELIOS-T-master_[non-isobaric]/planets/'+planet_name+'/cornerplot_'+planet_name+'_non_isobaric_'+model_name+'.png', format='png')
fig.savefig('/home/aline/Desktop/PhD/HELIOS-T-master_[non-isobaric]/planets/'+planet_name+'/cornerplot_'+planet_name+'_non_isobaric_'+model_name+'.eps', format='eps')
