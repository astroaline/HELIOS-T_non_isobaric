...
def corner(xs, xfull, xbin, yfull, ybin, ydata, x_err, y_err, range2, color_list, bins=20, range=None, weights=None, color="k",
    ...
    **hist2d_kwargs
    ...
    for i, x in enumerate(xs):
        ...
        if smooth1d is None:
            n, _, _ = ax.hist(x, bins=bins[i], weights=weights,
                              range=np.sort(range[i]), facecolor=fill_color, density=True, **hist_kwargs)
        else:
        ...
        if ini_guess is not None and ini_guess[i] is not None:
            ...
            ax2 = ax.twinx()
            ax2.plot(x_gauss, norm.pdf(x_gauss, mu, sigma), color='k', linestyle='-', linewidth=4)
    ...
    ax.text(0.85*wavelength_min+0.1*wavelength_max, 2.5*transit_max - 1.5*transit_min, 'Circles: '+planet_name+' data', color='r', fontsize=text_size)
